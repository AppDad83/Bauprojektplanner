...
import { Projekt, Fachfirma, Rechnung, Angebot, RechnungsTyp, AngebotStatus, FachfirmaAbnahme, AbnahmeArt, RechnungSicherheiten, BuergschaftDaten, Ansprechpartner } from '@/types';
...
const TabFachfirmen: React.FC<Props> = ({ projekt, onUpdate }) => {
  ...
  const handleEdit = (ff: Fachfirma) => {
    ...
    setFormData({
      ...
      notizen: ff.notizen || '',
      ansprechpartner: ff.ansprechpartner || []
    });
    setNeuerAnsprechpartner({ name: '', rolle: '', telefon: '', email: '', notizen: '' });
    setEditAnsprechpartnerId(null);
    setZeigeModal(true);
                ...
                {expandedId === ff.id && (
                  <div className="mt-6 border-t border-apleona-gray-200 pt-4 space-y-6">
                    ...
                    <div className="pt-4 border-t border-apleona-gray-200">
                      ...
                      {/* Firmenkontakt */}
                      {(ff.kontakt.telefon || ff.kontakt.email) && (
                        <div className="mb-3 text-sm text-apleona-gray-600">
                          <span className="font-medium">Firma: </span>
                          {ff.kontakt.telefon && <span className="mr-4">Tel: {ff.kontakt.telefon}</span>}
                          {ff.kontakt.email && <span>E-Mail: {ff.kontakt.email}</span>}
                        </div>
                      )}

                      {/* Ansprechpartner */}
                      {ff.ansprechpartner && ff.ansprechpartner.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                          {ff.ansprechpartner.map(ap => (
                            <div
                              key={ap.id}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="tel" value={formData.telefon} onChange={e => setFormData({ ...formData, telefon: e.target.value })} className="input-field" placeholder="Telefon der Firma" />
                </div>
                <div>
                  <label className="label">E-Mail (Zentrale)</label>
                  <input type="email" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} className="input-field" placeholder="E-Mail der Firma" />
                </div>
              </div>
